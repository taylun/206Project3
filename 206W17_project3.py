...
def get_user_tweets(twitter_handle):
	if twitter_handle in CACHE_DICTION:
		...
		search_results= api.user_timeline(screen_name= twitter_handle)
...
for tweet in umich_tweets:
	for lst in tweet["entities"]["user_mentions"]:
		...
		user_mention_particulars.append((user_info["id_str"], user_info["screen_name"], user_info["favourites_count"], user_info["description"]))
...
cur.execute(query4)
descriptions_fav_users= [str(x)[2:-3] for x in cur.fetchall()]
#print(descriptions_fav_users[0])
...
query5= "SELECT screen_name FROM Users INNER JOIN Tweets ON Users.user_id= Tweets.user_id WHERE Tweets.retweets> 50"
cur.execute(query5)
...
q= "SELECT Users.screen_name, Tweets.text FROM Tweets INNER JOIN Users ON Tweets.user_id = Users.user_id"
cur.execute(q)
list_of_tups= cur.fetchall()

first_dict= {}

for (x,y) in list_of_tups:
